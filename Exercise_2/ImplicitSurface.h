...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double x = _x.x() - m_center.x();
		double y = _x.y() - m_center.y();
		double z = _x.z() - m_center.z();
		return pow(x, 2) + pow(y, 2) + pow(z, 2) - pow(m_radius, 2);
	}
...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double y = _x.y() - m_center.y();
		double z = _x.z() - m_center.z();
		double s1 = pow(x,2)+pow(y,2)+pow(z,2)+pow(m_radius,2) - pow(m_a,2);
		double s2 = pow(m_radius,2)*(pow(x,2)+pow(y,2));
		return pow(s1,2)-4*s2;
	}
